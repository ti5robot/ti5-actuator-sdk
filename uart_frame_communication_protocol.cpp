...
uint32_t UFCP::calcCRC32(uint32_t crc, uint8_t *string, uint32_t size)
{
    while(size--)
...
int32_t UFCP::sendCommand(uint8_t numOfActuator, uint8_t *canIdList, uint8_t *commandList, uint32_t *parameterList)
{
...
//            qDebug()<<commandList[i];
    ...
    *(uint32_t *)(buf+index) = this->calcCRC32(0,buf,index);
    //    qDebug()<<*(uint32_t *)(buf+index);
    index+=4;
    char uartTxBuf[2000];
    uint32_t byteToWrite = MemToStrStream(uartTxBuf,(char*)buf,index,'<','\n');
    ulong writeSize=0;
    if(this->write(uartTxBuf, byteToWrite,&writeSize) != true)
    {
